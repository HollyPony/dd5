 ...
 *
 * Exposed members on returned enum proxy:
 * - Known enum keys (from `target`)
 * - `hasValue(value): boolean` helper to check if a value belongs to the enum
 *
 * @param {object} target - Object to freeze and proxy.
 * @param {string} [objectName] - Object display name used in thrown errors.
 * @returns {object & { hasValue(value: unknown): boolean }} Frozen strict proxy.
 * @throws {ReferenceError} If target is invalid or an unknown property is accessed.
 */
export const Enum = (target, objectName = inferEnumName(target)) => {
  ...
  const frozenTarget = f(target)
  const values = new Set(Object.values(frozenTarget))

  return new Proxy(frozenTarget, { // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy
    get(target, prop, receiver) { // receiver is the `this`
      if (Object.hasOwn(target, prop)) return Reflect.get(target, prop, receiver)

      if (prop === 'hasValue') return (value) => values.has(value)
